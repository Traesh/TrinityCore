...
void ScenarioMgr::LoadDBCData()
{
    ...
    for (uint32 scenarioId = 0; scenarioId < sScenarioStore.GetNumRows(); ++scenarioId)
    {
        ScenarioEntry const* scenario = sScenarioStore.LookupEntry(scenarioId);
        if (!scenario)
            continue;
        ...
        uint8 maxStepIndex = 0;

        for (uint32 stepId = 0; stepId < sScenarioStepStore.GetNumRows(); ++stepId)
        {
            ScenarioStepEntry const* step = sScenarioStepStore.LookupEntry(stepId);
            if (!step || step->ScenarioID != scenario->ID)
                continue;

            data->Steps[step->StepIndex] = step;
            maxStepIndex = std::max(maxStepIndex, step->StepIndex);
        }

        bool hasGapBetweenIndex = false;
        for (uint8 i = 0; i <= maxStepIndex; ++i)
        {
            if (data->Steps.find(i) == data->Steps.end())
            {
                hasGapBetweenIndex = true;
                break;
            }
        }

        if (hasGapBetweenIndex)
        {
            TC_LOG_DEBUG("server.loading", "ScenarioMgr::LoadDBCData: find gap between scenario steps index, would cause problem later, scenario ignored (Id: %u)!", scenarioId);
            continue;
        }

        data->maxStepIndex = maxStepIndex;
